...
root=/opt/smawell/shopify
backend=storefront-backend
service=smawell-store-api
exec 9>/var/lock/smawell-deploy.lock
flock -w 600 9
backup="/opt/smawell/backups/actions-store-$(date -u +%Y%m%dT%H%M%S)-$$"
mkdir -p "$backup"
...
for attempt in $(seq 1 20); do
  if curl -fsS "http://127.0.0.1:5301/api/health" > /dev/null; then
